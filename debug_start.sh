...
# 将APP_ENV抛出，本脚本的子进程将全部生效，但在全局不会生效
export APP_ENV=${APP_ENV}

# 如果ctrl+c，则退出另外2个后台进程
function exit_handler()
{
    echo -e '\n脚本被中断，触发终止函数，后台进程将被杀掉'
    sh ${SCRIPT_PATH}/debug_stop.sh
    exit
}
...
# 后台进程不打印日志到控制台
export IS_DEBUG=0
nohup ${PYTHON3_BIN}/python3 ${SCRIPT_PATH}/manage.py server1 > ${SCRIPT_PATH}/log 2>&1 &

# 前台进程打印日志到控制台
export IS_DEBUG=1
${PYTHON3_BIN}/python3 ${SCRIPT_PATH}/manage.py runserver 0.0.0.0:${PORT}
